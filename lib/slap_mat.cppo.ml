...
let of_array_c kind aa =
  let m, n = match dim_array_array aa with
    | None -> (0, 0)
    | Some (m, n) -> (m, n) in
  MAT (unsafe_of_array kind (Slap_size.__unexpose m)
                       (Slap_size.__unexpose n) aa)

let of_list_c kind ll =
  let m, n = match dim_list_list ll with
    | None -> (0, 0)
    | Some (m, n) -> (m, n) in
  MAT (unsafe_of_list kind (Slap_size.__unexpose m)
                      (Slap_size.__unexpose n) ll)

let of_bigarray_c ?(share=false) ba =
  let m = Slap_size.__unexpose (Array2.dim1 ba) in
