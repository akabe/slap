...
(** [gemv ?beta ?y ~trans ?alpha a x] executes
    ...
    @param trans the transpose flag for [a]:
      - If [trans] = {!Common.normal}, then [OP(a)] = [a];
      - If [trans] = {!Common.trans}, then [OP(a)] = [a^T];
      - If [trans] = {!Common.conjtr}, then [OP(a)] = [a^H]
...
(** [trmv ~trans ?diag ?up a x] executes [x := OP(a) * x].
    ...
    @param trans the transpose flag for [a]:
      ...
      - If [trans] = {!Common.conjtr}, then [OP(a)] = [a^H]
      (the conjugate transpose of [a]).
    @param diag default = {!Common.non_unit}
      - If [diag] = {!Common.unit}, then [a] is unit triangular;
      - If [diag] = {!Common.non_unit}, then [a] is not unit triangular.
    @param up default = {!Common.upper}
      - If [up] = {!Common.upper},
        then the upper triangular part of [a] is used;
      - If [up] = {!Common.lower},
...
(** [gemm ?beta ?c ~transa ?alpha a ~transb b] executes
    ...
    @param transa the transpose flag for [a]:
      - If [transa] = {!Common.normal}, then [OP(a)] = [a];
      - If [transa] = {!Common.trans}, then [OP(a)] = [a^T];
      - If [transa] = {!Common.conjtr}, then [OP(a)] = [a^H]
      (the conjugate transpose of [a]).
    @param alpha default = [1.0]
    @param transb the transpose flag for [b]:
      - If [transb] = {!Common.normal}, then [OP(b)] = [b];
      - If [transb] = {!Common.trans}, then [OP(b)] = [b^T];
      - If [transb] = {!Common.conjtr}, then [OP(b)] = [b^H]
      (the conjugate transpose of [b]).
 *)

val symm :
  side:('k, 'm, 'n) Common.side ->
  ?up:[< `U | `L ] Common.uplo ->
  ?beta:num_type ->
  ?c:('m, 'n, 'c_cd) mat ->
  ?alpha:num_type ->
...
(** [symm ~side ?up ?beta ?c ?alpha a b] executes
    ...
    - [c := alpha * a * b + beta * c] (if [side] = {!Common.left}) or
    - [c := alpha * b * a + beta * c] (if [side] = {!Common.right})
...
(** [trmm ~side ?up ~transa ?diag ?alpha ~a b] executes
    ...
    - [b := alpha * OP(a) * b] (if [side] = {!Common.left}) or
    - [b := alpha * b * OP(a)] (if [side] = {!Common.right})
...
(** [trsm ~side ?up ~transa ?diag ?alpha ~a b] solves a system of linear
    ...
    - [OP(a) * x = alpha * b] (if [side] = {!Common.left}) or
    - [x * OP(a) = alpha * b] (if [side] = {!Common.right})
...
val syrk :
  ...
  ?beta:num_type ->
  ?c:('n, 'n, 'c_cd) mat ->
  trans:('a_n * 'a_k, 'n * 'k, _) Common.trans2 ->
  ?alpha:num_type ->
  ('a_n, 'a_k, 'a_cd) mat -> ('n, 'n, 'c_cd) mat
(** [syrk ?up ?beta ?c ~trans ?alpha a] executes

    - [c := alpha * a * a^T + beta * c] (if [trans] = {!Common.normal}) or
    - [c := alpha * a^T * a + beta * c] (if [trans] = {!Common.trans})
...
(** [syr2k ?up ?beta ?c ~trans ?alpha a b] computes
    ...
    - [c := alpha * a * b^T + alpha * b * a^T + beta * c]
      (if [trans] = {!Common.normal}) or
    - [c := alpha * a^T * b + alpha * b^T * a + beta * c]
      (if [trans] = {!Common.trans})
...
(** [lange ?norm ?work a]
    ...
    @param norm default = {!Common.norm_1}.
      - If [norm] = {!Common.norm_1}, the one norm is returned;
      - If [norm] = {!Common.norm_inf}, the infinity norm is returned;
      - If [norm] = {!Common.norm_amax}, the largest absolute value of
        elements in matrix [a] (not a matrix norm) is returned;
      - If [norm] = {!Common.norm_frob}, the Frobenius norm is returned.
    @param work default = an optimum-length vector.
 *)

val lauum : ?up:[< `U | `L ] Common.uplo -> ('n, 'n, 'cd) mat -> unit
(** [lauum ?up a] computes

    - [U * U^T] where [U] is the upper triangular part of matrix [a]
      if [up] is {!Common.upper}.
    - [L^T * L] where [L] is the lower triangular part of matrix [a]
      if [up] is {!Common.lower}.
...
val getrf :
  ?ipiv:(('m, 'n) Size.min, cnt) Common.int32_vec ->
  ('m, 'n, 'cd) mat ->
...
val getrs :
  ?ipiv:(('n, 'n) Size.min, cnt) Common.int32_vec ->
  trans:('n * 'n, 'n * 'n, _) trans3 ->
  ('n, 'n, 'a_cd) mat ->
...
(** [sysv ?up ?ipiv ?work a b] solves systems of linear equations [a * x = b]
    ...
    - If [up] = {!Common.upper}, then [a = U * D * U^T]
    - If [up] = {!Common.lower}, then [a = L * D * L^T]
    where [U] and [L] are the upper and lower triangular matrices, respectively.
...
val gels_opt_lwork :
  trans:('am * 'an, 'm * 'n, _) Common.trans2 ->
  ('am, 'an, 'a_cd) mat ->
...
(** [gels ?work ~trans a b] solves an overdetermined or underdetermined system
    ...
    - If [trans] = {!Common.normal} and ['m >= 'n]: find the least square
      solution to an overdetermined system by minimizing [||b - A * x||^2].
    - If [trans] = {!Common.normal} and ['m < 'n]: find the minimum norm
      solution to an underdetermined system [a * x = b].
    - If [trans] = {!Common.trans}, and ['m >= 'n]: find the minimum norm
      solution to an underdetermined system [a^H * x = b].
    - If [trans] = {!Common.trans} and ['m < 'n]: find the least square
