...
val copy_diag_rect : ('m, 'n, 'cd) mat -> (('m, 'n) Size.min, 'cnt) vec
...
val of_array_c :
  num_type array array -> (num_type, prec, 'cnt) Mat.dyn
...
val of_list_c :
  num_type list list -> (num_type, prec, 'cnt) Mat.dyn
...
val map :
  (num_type -> num_type) -> ?b:('m, 'n, 'b_cd) mat ->
  ('m, 'n, 'a_cd) mat -> ('m, 'n, 'b_cd) mat

val mapi :
  (int -> int -> num_type -> num_type) -> ?b:('m, 'n, 'b_cd) mat ->
  ('m, 'n, 'a_cd) mat -> ('m, 'n, 'b_cd) mat

val fold_left :
  ('accum -> ('m, 'x_cd) vec -> 'accum) ->
  'accum ->
  ('m, 'n, 'a_cd) mat ->
  'accum
...
(** [gemm_diag ?beta ?y ~transa ?alpha a ~transb b] executes
    ...
    @param transa the transpose flag for [a]:
       - if {!Common.normal}, then [OP(a)] = [a];
       - if {!Common.trans}, then [OP(a)] = [a^T];
       - if {!Common.conjtr}, then [OP(a)] = [a^H].
    @param transb the transpose flag for [b]:
       - if {!Common.normal}, then [OP(b)] = [b];
       - if {!Common.trans}, then [OP(b)] = [b^T];
       - if {!Common.conjtr}, then [OP(b)] = [b^H].
