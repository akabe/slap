...
let sbmv ~k ?y a ?(up = Common.__unexpose_uplo 'U')
    ?(alpha = 1.0) ?(beta = 0.0) x =
  let sbs, n, ar, ac, a = Mat.__expose a in
  let n', incx, x = Vec.__expose x in
  assert(n = n' && sbs = Size.syband_dyn n k);
  let incy, y = Vec.opt_vec_alloc prec n y in
  if Size.nonzero n && Size.nonzero k
...
let ger ?(alpha = 1.0) x y a =
  let m, incx, x = Vec.__expose x in
  let n, incy, y = Vec.__expose y in
...
let lansy_min_lwork n norm =
  let lwork = match Common.char_of_norm norm with
    | 'I' | 'O' -> Size.__expose n
    | _ -> 0 in
  Size.__unexpose lwork

let lansy
    ?(up = Common.__unexpose_uplo 'U')
    ?(norm = Common.__unexpose_norm 'O')
    ?work a =
  let n, n', ar, ac, a = Mat.__expose a in
  assert(n = n');
  if Size.nonzero n then begin
    let min_lwork = lansy_min_lwork n norm in
    let _, work = Vec.__alloc_work prec work ~loc:"Slap.XSDCZ.lansy"
...
type 'n orgqr_min_lwork = (Size.one, 'n) Size.max

let orgqr_min_lwork ~n = Size.max Size.one n

let orgqr_check_size loc k m n =
  let k = Size.__expose k in
  let m = Size.__expose m in
  let n = Size.__expose n in
  if m < n
  then Misc.invalid_argf "%s: dim1(a)=%d, dim2(a)=%d \
                               (unsatisfied: dim1(a) >= dim2(a))" loc m n ();
  if n < k
  then Misc.invalid_argf "%s: dim2(a)=%d, dim(tau)=%d \
                               (unsatisfied: dim2(a) >= dim(tau))" loc n k ()

let orgqr_opt_lwork_aux ~tau a =
  assert(Vec.check_cnt tau);
  let k, _, tau = Vec.__expose tau in
  let m, n, ar, ac, a = Mat.__expose a in
  ...
  if i = 0 then Size.__unexpose (int_of_float work.{1})
...
let orgqr_dyn ?work ~tau a =
  assert(Vec.check_cnt tau);
  let k, _, ba_tau = Vec.__expose tau in
  let m, n, ar, ac, ba_a = Mat.__expose a in
  orgqr_check_size "orgqr_dyn" k m n;
  if Size.nonzero k && Size.nonzero m && Size.nonzero n
  then begin
    let loc = "Slap.XSDCZ.orgqr_dyn" in
    let lwork, work = Vec.__alloc_work prec work ~loc
...
let check_ormqr ~loc ~side ~r ~m ~n ~k ~k' =
  assert(k = k');
  let r = Size.__expose r in
  let m = Size.__expose m in
  let n = Size.__expose n in
  let k = Size.__expose k in
  assert(match Common.char_of_side side with
      | 'L' -> r = m
      | _ -> r = n);
  if k > m
  then Misc.invalid_argf "%s: dim(tau)=%d, dim1(c)=%d \
...
let ormqr_min_lwork ~side ~m ~n =
  begin
    match Common.char_of_side side with
    | 'L' -> max 1 (Size.__expose n)
    | _ -> max 1 (Size.__expose m)
  end
  |> Size.__unexpose
...
let ormqr_opt_lwork_aux ~side ~trans ~tau a c =
  ...
  let k, _, tau = Vec.__expose tau in
  let r, k', ar, ac, a = Mat.__expose a in
  let m, n, cr, cc, c = Mat.__expose c in
...
let ormqr_dyn ~side ~trans ?work ~tau a c =
  ...
  if Size.nonzero r && Size.nonzero m
     && Size.nonzero n && Size.nonzero k
  then begin
    let lwork, work =
      Vec.__alloc_work prec work ~loc
...
type 'n gecon_min_lwork = (Size.four, 'n) Size.mul

let gecon_min_lwork n = Size.mul Size.four n

type 'n gecon_min_liwork = 'n

let gecon_min_liwork n = n

let gecon ?(norm = Common.__unexpose_norm 'O') ?anorm ?work ?iwork aa =
  let n, n', ar, ac, a = Mat.__expose aa in
  assert(n = n');
  if Size.nonzero n then begin
    let loc = "Slap.XSDCZ.gecon" in
    let min_lwork = gecon_min_lwork n in
    let _, work = Vec.__alloc_work prec work ~loc
        ~min_lwork ~opt_lwork:min_lwork in
    let min_liwork = gecon_min_liwork n in
    let _, iwork = Vec.__alloc_work int32 iwork ~loc
...
type 'n sycon_min_lwork = (Size.two, 'n) Size.mul

let sycon_min_lwork n = Size.mul Size.two n

type 'n sycon_min_liwork = 'n

let sycon_min_liwork n = n

let sycon ?(up = Common.__unexpose_uplo 'U') ?ipiv ?anorm ?work ?iwork aa =
  let n, n', ar, ac, a = Mat.__expose aa in
  assert(n = n');
  if Size.nonzero n then begin
    ...
    let ipiv =
      if ipiv = None
      then sytrf ~up ~work:(Vec.__unexpose min_lwork 1 work) aa
           |> Vec.__expose
           |> (fun (_, _, ipiv) -> ipiv)
      else Vec.opt_cnt_vec_alloc int32 n ipiv in
    let anorm = match anorm with
      | None -> lange aa
      | Some anorm -> anorm in
...
type 'n pocon_min_lwork = (Size.three, 'n) Size.mul

let pocon_min_lwork n = Size.mul Size.three n

type 'n pocon_min_liwork = 'n

let pocon_min_liwork n = n

let pocon ?(up = Common.__unexpose_uplo 'U') ?anorm ?work ?iwork aa =
  let n, n', ar, ac, a = Mat.__expose aa in
  assert(n = n');
  if Size.nonzero n then begin
...
let gelsx_err loc i =
  assert(i <> 0);
  if i > 0
  then Misc.failwithf "%s: failed to converge on off-diagonal \
                            element %d" loc i ()
  else internal_error loc i
...
type ('m, 'n, 'nrhs) gelsy_min_lwork =
  (((('m, 'n) Size.min,
     (Size.three, 'n) Size.mul) Size.add,
    Size.one)
     Size.add,
   ((Size.two, ('m, 'n) Size.min) Size.mul,
    'nrhs) Size.add)
    Size.max
...
let gelsy_opt_lwork_aux a b =
  ...
  if i = 0 then int_of_float work.{1} |> Size.__unexpose
...
let gelsy aa ?(rcond = -1.0) ?jpvt ?work bb =
  ...
  then begin
    ...
    let jpvt = match jpvt with
      | Some jpvt ->
        assert(Vec.check_cnt jpvt);
        let n'', _, jpvt = Vec.__expose jpvt in
        assert(n = n'');
        jpvt
      | None ->
        let jpvt = Array1.create int32 fortran_layout (Size.to_int n) in
...
let gelsd_min_lwork ~m ~n ~nrhs =
  Lacaml.XSDCZ.gelsd_min_lwork
    ~m:(Size.__expose m) ~n:(Size.__expose n)
    ~nrhs:(Size.__expose nrhs)
  |> Size.__unexpose
...
let gelsd_opt_lwork_aux a b =
  ...
  if i = 0 then
    ...
    Size.max
      (Size.__unexpose (int_of_float work.{1}))
...
let gelsd_min_iwork m n =
  Lacaml.XSDCZ.gelsd_min_iwork (Size.__expose m) (Size.__expose n)
  |> Size.__unexpose
...
let gelsd aa ?(rcond = -1.0) ?s ?work ?iwork bb =
  ...
  let n', nrhs, br, bc, b = Mat.__expose bb in
  assert(n = n');
  let mn = Size.min m n in
  if Size.nonzero mn && Size.nonzero nrhs then begin
    let loc = "Slap.XSDCZ.gelsd" in
    let s = Vec.opt_cnt_vec_alloc prec mn s in
    let min_liwork = gelsd_min_iwork m n in
    let _, iwork = Vec.__alloc_work prec iwork ~loc
        ~min_lwork:min_liwork ~opt_lwork:min_liwork in
    let lwork, work = Vec.__alloc_work prec work ~loc
...
let gelss aa ?(rcond = -1.0) ?s ?work bb =
  ...
  assert(n = n');
  let mn = Size.min m n in
  if Size.nonzero mn && Size.nonzero nrhs
  then begin
...
external direct_gesvd :
  jobu : (_, _, _, _, _) Common.svd_job ->
  jobvt : (_, _, _, _, _) Common.svd_job ->
  m : _ Size.t ->
  n : _ Size.t ->
  ar : int ->
  ac : int ->
...
let gesvd_calc_sizes m n jobu jobvt =
  let open Size in
  let min_mn = min m n in
  let job_size k job = match Common.char_of_svd_job job with
    | 'A' -> __expose k
    | 'S' -> __expose min_mn
    | _ -> 0 in
...
let gesvd_opt_lwork_aux ~jobu ~jobvt ?s ?u ?vt a =
  ...
  let ur, uc, u = Mat.opt_mat_alloc prec m u_cols u in
  let vtr, vtc, vt = Mat.opt_mat_alloc prec vt_rows n vt in
  let s = Vec.opt_cnt_vec_alloc prec min_mn s in
  let work = Array1.create prec fortran_layout 1 in
  let i =
...
let gesvd ~jobu ~jobvt ?s ?u ?vt ?work a_mat =
  let m, n, ar, ac, a = Mat.__expose a_mat in
  let min_mn, u_cols, vt_rows = gesvd_calc_sizes m n jobu jobvt in
  let ur, uc, u = Mat.opt_mat_alloc prec m u_cols u in
  let vtr, vtc, vt = Mat.opt_mat_alloc prec vt_rows n vt in
  let s = Vec.opt_cnt_vec_alloc prec min_mn s in
  let u_mat = Mat.__unexpose m u_cols ur uc u in
  let vt_mat = Mat.__unexpose vt_rows n vtr vtc vt in
  let s_vec = Vec.__unexpose min_mn 1 s in
...
let gesdd_opt_lwork ~jobz ?s ?u ?vt ?iwork a =
  ...
  let min_mn = Size.min m n in
  let u_cols, vt_rows = gesdd_calc_sizes m n jobz in
  let ur, uc, u = Mat.opt_mat m (S.__unexpose u_cols) u in
  let vtr, vtc, vt = Mat.opt_mat (S.__unexpose vt_rows) n vt in
  I.gesdd_opt_lwork ~m:(S.__expose m) ~n:(S.__expose m)
    ~jobz:(lacaml_svd_job jobz) ?s:(Vec.opt_cnt_vec min_mn s)
    ?iwork:(Vec.opt_work iwork)
...
let gesdd ~jobz ?s ?u ?vt ?work ?iwork a =
  ...
  let _gesdd ?ur ?uc ?u ?vtr ?vtc ?vt () =
    I.gesdd ~m:(S.__expose m) ~n:(S.__expose n) ~jobz:(lacaml_svd_job jobz)
      ?s:(Vec.opt_cnt_vec min_mn s) ?work:(Vec.opt_work work)
      ?iwork:(Vec.opt_work iwork) ?ur ?uc ?u ?vtr ?vtc ?vt ~ar ~ac a
  in
  let opt_mat_alloc m n a = Mat.opt_mat_alloc prec m n a in
...
let geev_opt_lwork ?vl ?vr ?wr ?wi a =
  ...
  I.geev_opt_lwork ~n:(S.__expose n) ?vlr ?vlc ?vl ?vrr ?vrc ?vr
    ?wr:(Vec.opt_cnt_vec n wr) ?wi:(Vec.opt_cnt_vec n wi) ~ar ~ac a
  |> Size.unsafe_of_int
...
let geev ?work ?vl ?vr ?wr ?wi a =
  ...
  let vl, wr, wi, vr = I.geev ~n:(S.__expose n)
      ?work:(Vec.opt_work work) ?vlr ?vlc ?vl ?vrr ?vrc ?vr
      ?wr:(Vec.opt_cnt_vec n wr) ?wi:(Vec.opt_cnt_vec n wi)
      ~ar ~ac a in
...
let syev ?vectors ?up ?work ?w a =
  let n, n', ar, ac, a = M.__expose a in
  assert(n = n');
  let w = Vec.opt_cnt_vec_alloc prec n w in
  ignore (I.syev ~n:(S.__expose n) ?vectors ?up
            ?work:(Vec.opt_work work) ~w ~ar ~ac a);
  V.__unexpose n 1 w
...
let syevd ?vectors ?up ?work ?iwork ?w a =
  ...
  let w = I.syevd ~n:(S.__expose n) ?vectors ?up
      ?work:(Vec.opt_work work) ?iwork:(Vec.opt_work iwork)
      ?w:(Vec.opt_cnt_vec n w) ~ar ~ac a in
  V.__unexpose n 1 w
...
let sbev ~kd ?z ?up ?work ?w ab =
  ...
  let zr, zc, z = Mat.opt_mat_alloc prec n n z in
  let w = I.sbev ~n:(S.__expose n) ~kd:(S.__expose kd) ~zr ~zc ~z ?up
      ?work:(Vec.opt_work work) ?w:(Vec.opt_cnt_vec n w)
      ~abr ~abc ab in
  V.__unexpose n 1 w
...
let syevr_dyn (type nn) ?vectors ?range ?up ?abstol ?work ?iwork ?w ?z ?isuppz
  ...
  let syevr k ?zr ?zc ?z ar ac a =
    I.syevr ~n:(S.__expose n) ?vectors ?range ?up ?abstol
      ?work:(Vec.opt_work work) ?iwork:(Vec.opt_work iwork)
      ?w:(Vec.opt_cnt_vec n w)
      ?isuppz:(Vec.opt_cnt_vec (Size.add k k) isuppz)
      ?zr ?zc ?z ~ar ~ac a in
...
let sygv ?vectors ?up ?work ?w ?itype a b =
  ...
  let n'', n''', br, bc, b = M.__expose b in
  assert(n = n' && n = n'' && n = n''');
  let w = I.sygv ~n:(S.__expose n) ?vectors ?up ?work:(Vec.opt_work work)
                 ?w:(Vec.opt_cnt_vec n w) ?itype ~ar ~ac a ~br ~bc b in
  V.__unexpose n 1 w
...
let sbgv ~ka ~kb ?z ?up ?work ?w ab bb =
  ...
  let w = I.sbgv ~n:(S.__expose n) ~ka:(S.__expose ka) ~kb:(S.__expose kb)
      ~zr ~zc ~z ?up ?work:(Vec.opt_work work)
      ?w:(Vec.opt_cnt_vec n w) ~ar:abr ~ac:abc ab ~br:bbr ~bc:bbc bb in
  V.__unexpose n 1 w
