...
(** [ormqr_dyn ~side ~trans ?work ~tau a c] multiplies a matrix [c] by the
    ...
    - [Q * c] if [side] = {!Common.left} and
                 [trans] = {!Common.normal};
    - [Q^T * c] if [side] = {!Common.left} and
                   [trans] = {!Common.trans};
    - [c * Q] if [side] = {!Common.right} and
                 [trans] = {!Common.normal};
    - [c * Q^T] if [side] = {!Common.right} and
                   [trans] = {!Common.trans}.
...
(** [gesvd ?jobu ?jobvt ?s ?u ?vt ?work a] computes the singular value
    ...
    @param jobu the SVD job flag for [u]:
      - If [jobu] = {!Common.svd_all}, then all ['m] columns of [U] are
        returned in [u]. (['u_cols] = ['m].)
      - If [jobu] = {!Common.svd_top}, then the first [('m, 'n) min]
        columns of [U] are returned in [u]. (['u_cols] = [('m, 'n) min].)
      - If [jobu] = {!Common.svd_overwrite}, then the first [('m, 'n) min]
        columns of [U] are overwritten on [a]. (['u_cols] = [z] since [u]
        is unused.)
      - If [jobu] = {!Common.svd_no}, then no columns of [U] are computed.
        (['u_cols] = [z].)
    @param jobvt the SVD job flag for [vt]:
      - If [jobvt] = {!Common.svd_all}, then all ['n] rows of [V^T] are
        returned in [vt]. (['vt_rows] = ['n].)
      - If [jobvt] = {!Common.svd_top}, then the first [('m, 'n) min]
        rows of [V^T] are returned in [vt]. (['vt_rows] = [('m, 'n) min].)
      - If [jobvt] = {!Common.svd_overwrite}, then the first [('m, 'n) min]
        rows of [V^T] are overwritten on [a]. (['vt_cols] = [z] since [vt]
        is unused.)
      - If [jobvt] = {!Common.svd_no}, then no columns of [V^T] are
        computed. (['vt_cols] = [z].)
    @param s    a return location for singular values.
    ...
    ({b Note}: [jobu] and [jobvt] cannot both be {!Common.svd_overwrite}.)
 *)
...
(** [gesdd ~jobz ?s ?u ?vt ?work ?iwork a] computes the singular value
    ...
    - If [jobz] is {!Common.svd_no}, no singular vectors are computed.
