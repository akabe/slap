...
val get_transposed_dim :
  ('m * 'n, 'k * 'l, _) trans ->
  'm Size.t -> 'n Size.t -> 'k Size.t * 'l Size.t
(** [get_transposed_dim trans m n] returns
    - [(m * n)] if [trans] is {!Common.normal};
    - [(n * m)] if [trans] is {!Common.trans} or {!Common.conjtr}. *)
...
(** [('k, 'm, 'n) side] is the type of left- and right-multiplication flags.
    ...
    - When [A] is multiplied from the left by [B] (i.e., [A*B]), ['k] is equal
      to ['m]; therefore the type of {!Common.left} is [('m, 'm, 'n) side].
    - Conversely, if [A] is right-multiplied by [B] (i.e., [B*A]), ['k] is equal
      to ['n]. Thus, the flag {!Common.right} is given the type
      [('n, 'm, 'n) side]. *)
...
(** Values of this type are {!Common.norm_1} and {!Common.norm_inf}.
 *)
...
type (+'n, +'cnt_or_dsc) int_vec = ('n, int, int_elt, 'cnt_or_dsc) Vec.t
