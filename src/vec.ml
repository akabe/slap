...
type (+'n, 'num, 'prec, +'cnt_or_dsc) t =
  'n Size.t (* the number of elements in a vector (>= 0) *)
...
external create_array1 :
  ('a, 'b) kind -> 'n Size.t -> ('a, 'b, fortran_layout) Array1.t
  = "slap_vec_create_array1"

let check_vec (n, incx, x) =
  (Size.__expose n - 1) * abs incx + 1 <= Array1.dim x

let check_cnt (n, incx, x) =
  Size.__expose n = Array1.dim x && incx = 1
...
let __unexpose_with_ofs n ofsx incx x =
  let x =
    if Size.iszero n then x
    else Array1.sub x ofsx ((Size.__expose n - 1) * abs incx + 1) in
  __unexpose n incx x

let __alloc_work ~loc ~min_lwork ~opt_lwork kind =
  let m = Size.__expose min_lwork in
  let n = Size.__expose opt_lwork in
  function
  | None -> (n, Array1.create kind fortran_layout n)
  | Some ((k, _, work) as v) ->
    let lwork = Size.__expose k in
...
let index n incx i =
  let j = if incx > 0 then 1 else Size.__expose n in
  (i - j) * incx + 1

let get_dyn (n, incx, x) i =
  if i < 1 || i > Size.__expose n then invalid_arg "Slap.Vec.get_dyn";
  Array1.get x (index n incx i)

let set_dyn (n, incx, x) i a =
  if i < 1 || i > Size.__expose n then invalid_arg "Slap.Vec.set_dyn";
...
let replace_dyn (n, incx, x) i f =
  if i < 1 || i > Size.__expose n then invalid_arg "Slap.Vec.replace_dyn";
  let j = index n incx i in
  Array1.unsafe_set x j (f (Array1.unsafe_get x j))

(** {2 Iterators} *)

let index_hd n incx = if incx > 0 then 1 else (1 - Size.__expose n) * incx + 1

let mapi kind f ?y (n, incx, x) =
  let incy, y = opt_vec_alloc kind n y in
  let rec loop count ix iy =
    if count <= Size.__expose n then
      begin
...
let fold_lefti f init (n, incx, x) =
  let rec loop count i acc =
    if count > Size.__expose n then acc else
      begin
...
let fold_righti f vx init =
  let n = Size.__expose (dim vx) + 1 in
...
let for_all p (n, incx, x) =
  let rec loop count i =
    if count > Size.__expose n then true else
...
let subcntvec_dyn n ?(ofsx = 1) ((n', _, x) as vx) =
  assert(Size.__expose n' >= Size.__expose n && check_cnt vx);
  (n, 1, Array1.sub x ofsx (Size.__expose n))

let subdscvec_dyn nx ?(ofsx = 1) ?(incx = 1) (ny, incy, y) =
  if Size.__expose nx = 0 then (nx, 1, y)
  else begin
    let last_idx = Size.__expose nx - 1 in
    ...
    (nx, incz, Array1.sub y ofsz ((Size.__expose nx - 1) * abs incz + 1))
  end
...
let hd x =
  let n = Size.to_int (dim x) in
  assert(n >= 1);
  unsafe_get x 1

let hd_dyn x =
  if Size.to_int (dim x) >= 1 then hd x
...
let last_dyn x =
  if Size.to_int (dim x) >= 1 then last x
...
let tl ?(share = false) ((n, _, _) as x) =
  assert(Size.to_int n >= 1);
  internal_tl ~share (Size.pred n) x

let tl_dyn ?(share = false) ((n, _, _) as x) =
  if Size.to_int n >= 1
  then internal_tl ~share (Size.pred_dyn n) x
...
let inits ?(share = false) ((n, _, _) as x) =
  assert(Size.to_int n >= 1);
  internal_inits ~share (Size.pred n) x

let inits_dyn ?(share = false) ((n, _, _) as x) =
  if Size.to_int n >= 1
  then internal_inits ~share (Size.pred_dyn n) x
...
let append (m, incx, x) (n, incy, y) =
  let k = Size.add m n in
  let z = create_array1 (Array1.kind x) k in
  copy_stub ~n:m ~ofsx:1 ~incx x ~ofsy:1 ~incy:1 z;
  copy_stub ~n ~ofsx:1 ~incx:incy y ~ofsy:(Size.__expose m + 1) ~incy:1 z;
  (k, 1, z)
...
let to_array (n, incx, x) =
  ...
  Array.init (Size.__expose n) (fun i -> x.{incx * i + ofsx})
...
let of_array_dyn kind n arr =
  if Size.__expose n <> Array.length arr then invalid_arg "Slap.Vec.of_array_dyn";
...
let of_list_dyn kind n lst =
  if Size.__expose n <> List.length lst then invalid_arg "Slap.Vec.of_list_dyn";
...
let of_bigarray_dyn ?(share=false) n ba =
  if Size.__expose n <> Array1.dim ba then invalid_arg "Slap.Vec.of_bigarray_dyn";
...
let of_array_c kind arr =
  let n = Size.__unexpose (Array.length arr) in
  VEC (unsafe_of_array kind n arr)

let of_list_c kind lst =
  let n = Size.__unexpose (List.length lst) in
  VEC (unsafe_of_list kind n lst)

let of_bigarray_c ?(share=false) ba =
  let n = Size.__unexpose (Array1.dim ba) in
