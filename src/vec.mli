...
(** Like {!Vec.get_dyn}, but size checking is not always performed. *)

val unsafe_set : ('n, 'num, 'prec, 'cd) t -> int -> 'num -> unit
(** Like {!Vec.set_dyn}, but size checking is not always performed. *)
...
val cons :
  ?y:('n Size.s, 'num, 'prec, 'y_cd) t ->
  'num ->
  ('n, 'num, 'prec, 'x_cd) t ->
  ('n Size.s, 'num, 'prec, 'y_cd) t
...
val tl :
  ?share:bool ->
  ('n Size.s, 'num, 'prec, 'x_cd) t -> ('n, 'num, 'prec, 'x_cd) t
...
val tl_dyn :
  ?share:bool ->
  ('n, 'num, 'prec, 'x_cd) t -> ('n Size.p, 'num, 'prec, 'x_cd) t
...
val shared_rev : ('n, 'num, 'prec, 'cd) t -> ('n, 'num, 'prec, dsc) t
(** [shared_rev (x1, x2, ..., xn)]
...
val __expose :
  ...
  'n Size.t * int * ('num, 'prec, fortran_layout) Array1.t
